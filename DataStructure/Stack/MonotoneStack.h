...
#ifndef ARCHIVES_MONOTONESTACK_H
#define ARCHIVES_MONOTONESTACK_H
...
template<class T, bool ascending = true>
class MonotoneStack {
private:
    Stack<T> store;
...
public:
    void push(const T &o) {
        while (!store.empty() && (ascending ? store.top() > o : store.top() < o)) {
            store.pop();
        }
        store.push(o);
    }
...
#endif //ARCHIVES_MONOTONESTACK_H
